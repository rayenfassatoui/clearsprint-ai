...
const DatabaseWithRestApi = ({
  ...
  return (
    ...
    >
      {/* SVG Paths  */}
      <svg
        className="h-full sm:w-full text-muted-foreground/20"
        width="100%"
        height="100%"
        viewBox="0 0 200 100"
      >
        ...
        <defs>
          ...
          <radialGradient id="db-blue-grad" fx="1">
            <stop offset="0%" stopColor={lightColor || 'var(--primary)'} />
      ...
      <div className="absolute bottom-10 flex w-full flex-col items-center">
        {/* bottom shadow */}
        <div className="absolute -bottom-4 h-[100px] w-[62%] rounded-lg bg-primary/10 blur-xl" />
        {/* box title */}
        <div className="absolute -top-3 z-20 flex items-center justify-center rounded-lg border bg-card px-2 py-1 sm:-top-4 sm:py-1.5 shadow-sm">
          <SparklesIcon className="size-3 text-primary" />
          <span className="ml-2 text-[10px] font-medium text-foreground">
            {title ? title : 'Data exchange using a customized REST API'}
          </span>
        </div>
        {/* box outter circle */}
        <div className="absolute -bottom-8 z-30 grid h-[60px] w-[60px] place-items-center rounded-full border-t bg-card font-semibold text-xs shadow-lg text-foreground">
          {circleText ? circleText : 'API'}
        </div>
        {/* box content */}
        <div className="relative z-10 flex h-[150px] w-full items-center justify-center overflow-hidden rounded-lg border bg-card/50 backdrop-blur-sm shadow-md">
          {/* Badges */}
          <div className="absolute bottom-8 left-12 z-10 h-7 rounded-full bg-card px-3 text-xs border flex items-center gap-2 shadow-sm">
            <HeartHandshakeIcon className="size-4 text-primary" />
            <span className="text-foreground">
              {buttonTexts?.first || 'Team Sync'}
            </span>
          </div>
          <div className="absolute right-16 z-10 hidden h-7 rounded-full bg-card px-3 text-xs sm:flex border items-center gap-2 shadow-sm">
            <Folder className="size-4 text-primary" />
            <span className="text-foreground">
              {buttonTexts?.second || 'Projects'}
            </span>
          </div>
          {/* Circles */}
          <motion.div

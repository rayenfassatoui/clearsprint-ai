...
- [x] 2.2 Server Action: connectJira() → store tokens in jira_tokens table. (Handled by better-auth social provider + lib/jira.ts)
- [x] 2.3 Dashboard button: "Connect Jira" → redirect OAuth → toast success.

### Epic 3: Doc Upload + Storage (Priority 2)
- [x] 3.1 shadcn Dropzone component in /dashboard.
- [x] 3.2 Server Action: uploadDoc() →S3 compliant api (minio).put(file) → store URL in new project row.
- [x] 3.3 PDF/TXT extract: fetch URL → pdf-parse or text() → store raw_text in projects.

### Epic 4: AI Backlog Generation (Priority 2)
- [x] 4.1 Server Action: generateBacklog(projectId) → OpenAI chat.completions (prompt: doc_text → JSON epics/tasks).
- [x] 4.2 Parse JSON → insert tickets (set parentId/orderIndex for hierarchy).
- [x] 4.3 Button on upload: "Generate" → optimistic UI + loading → cards render.

### Epic 5: Cards Visualization + Edit/Drag (Priority 3)
- [x] 5.1 Fetch /api/projects/[id]/tickets → render grid of shadcn Cards (hierarchy via parentId).
- [x] 5.2 Inline edit: title/desc → Server Action updateTicket().
- [x] 5.3 @dnd-kit: Drag/drop reorder → onDrop update orderIndex/parentId in DB.

### Epic 6: AI Tweaks (Priority 3)
- [x] 6.1 Global "Refine All": Server Action re-gen entire project → diff modal → apply/reject.
- [x] 6.2 Per-card: Modal "AI Tweak" → user prompt input → OpenAI edit single ticket → replace.
- [x] 6.3 Log prompt in ticket for history.

### Epic 7: Jira Push (Priority 4)
- [x] 7.1 Fetch user jira_tokens → list projects via API.
- [x] 7.2 Modal: Select jira_project_key → confirm.
- [x] 7.3 Server Action: pushToJira() → create epic → for each task/subtask POST /issue with parent=epicKey → update ticket.jiraId.
- [x] 7.4 Toast: "Pushed! View in Jira: [link]".

### Epic 8: History + Polish (Priority 5)
- [x] 8.1 Sidebar: List user projects → click load cards.
- [x] 8.2 shadcn Sonner for all toasts/errors.
- [x] 8.3 Zod validate all Server Actions/JSON from OpenAI.
- [x] 8.4 Responsive: Mobile cards stack, desktop 3-col grid.
- [x] 8.5 Loading: shadcn Skeleton on cards/gen.
